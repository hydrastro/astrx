...
/**
 * The onion bridge: a zero-dependency HTTP client for the standalone
 * onioncrawler JSON API. No curl, no Composer — a plain `file_get_contents`
 * over an http stream context with a short timeout. AstrX talks only to the
 * loopback engine; the engine owns the Tor SOCKS hop.
 *
 * SECURITY MODEL
 * --------------
 *   * The host + scheme come only from {@see OnionSearchConfig} (operator-
 *     trusted, normalised to an http(s) origin). The end user controls only `q`
 *     and `page`; `q` is rawurlencode()d and `page` is cast to int, so neither
 *     can escape the query string or reach the host/scheme. No SSRF surface.
 ...
 *   * Results are CRAWLED, UNTRUSTED .onion content. Every text field is
 ...
 *     authoritative boundary. Result URLs are reduced to a safe href (http/https
 *     only, else `#`) so a hostile `javascript:` URL can never land in an anchor.
 *
 * This class has NO AstrX dependencies beyond {@see OnionSearchConfig}, so it
 * can be exercised in isolation (see tests/bridge_test.php).
 */
final class OnionSearchClient
{
    public function __construct(private readonly OnionSearchConfig $config) {}
    ...
    public function search(string $query, int $page): array
    {
        ...
        return [
            ...
            // onioncrawler's API paginates with `per_page`, not `page_size`.
            'page_size' => max(1, self::i($data['per_page'] ?? null, $this->config->perPage())),
    ...
    private function mapResults(mixed $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $out[] = [
                ...
                // onioncrawler strips <mark> in its API already; we strip again
                // defensively so untrusted content can never carry markup through.
                'snippet' => self::cleanText(self::s($row['snippet'] ?? null)),
            ];
        }
        return $out;
    }
