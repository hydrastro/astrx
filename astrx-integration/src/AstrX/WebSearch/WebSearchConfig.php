...
/**
 * Clear-web search backend configuration.
 *
 * Bound to the 'WebSearchConfig' section of WebSearch.config.php via
 * #[InjectConfig] (the domain resolves to the parent namespace segment
 * 'WebSearch' since there is no WebSearchConfig.config.php — the same fallback
 * BotTrapConfig/ChatConfig rely on).
 *
 * The base URL is operator-controlled and MUST point at the localhost engine.
 * It is normalised here to an http(s) origin (trailing slash stripped); any
 * other scheme (file://, gopher://, …) is rejected back to the safe default so
 * a bad edit can never turn the bridge into an SSRF primitive. The request
 * builder only ever appends `/api/search?q=…&page=…`, and the end user controls
 * only q and page — never the host or scheme.
 */
final class WebSearchConfig
{
    /** Safe fallback origin: the loopback engine on its default port. */
    public const string DEFAULT_BASE_URL = 'http://127.0.0.1:8803';
