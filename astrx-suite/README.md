...
|----------------|:------:|--------------------|
| `crawlcore`    | ✅ done | globmatch, dedup, scheduler, traps, **hashing (SHA-1/SHA-256/MD5 + BLAKE2b)**, **INFLATE (DEFLATE/gzip/zlib decompressor)**, **`urlparse`** (shared `urllib.parse`/`posixpath` subset: quote/unquote, parse_qsl/urlencode, urlsplit/urljoin/normpath), **`json`** (a bounded, dependency-free RFC-8259 parser — the `json.loads` stand-in for the crawlers' structured-data recovery: order-preserving objects, duplicate-key-keeps-last, `\u` surrogate pairs, depth-capped against hostile input) — 37 tests; SimHash + hashes (incl. RFC 7693 BLAKE2b) + inflate + urlparse + json byte-identical to Python |
...
| `websearch`    | 🚧 | **SSRF gate as a type** — `SafeIp` wraps an `IpAddr` only after `ip_is_internal` clears it (loopback/private/link-local/reserved/multicast/unspecified, IPv4-mapped unwrapped, fail-closed), so the net-tier connect taking a `&SafeIp` makes an SSRF to an internal address a *compile* error; cross-checked byte-identical to Python over a 60+-case IPv4/IPv6 special-range corpus. Plus **`dedup`** (FNV-1a word-bigram-shingled 64-bit SimHash), the **URL canonicalizer** (`canonicalize`/`host_of`/`authority_of`/`is_http_url`/`in_scope`: lower-case scheme+host, default-port drop, userinfo/fragment strip, RFC-3986 dot-segments, multi-slash collapse, query sort, IPv6 brackets) on the now-shared `crawlcore::urlparse`, and the **robots.txt parser** (UA-group precedence, Allow/Disallow with `*`/`$`, longest-match + Allow tie-break, Crawl-delay, over crawlcore's ReDoS-safe glob), and now the **pure HTTP wire core** where the gate reaches the socket — `vet_addrs` turns a host's resolved addresses into `SafeIp`s or refuses the whole host (any-internal → blocked, first offender named; DNS-rebind-safe pinning), the `allow_hosts` authority exemption (the sole, audit-visible way an internal address becomes connectable), `Content-Type`/`Content-Encoding` handling (gzip/deflate/zlib over crawlcore's INFLATE, bomb-capped), charset body decode, and the HTTP/1.1 request/response/chunked helpers — all cross-checked byte-identical to Python (`vet`, content-type, decompress, decode). Plus the **`net` tier** — the async SSRF-checked fetch: a TTL DNS cache, `resolve_checked` (resolve → `vet_addrs` → `Vec<SafeIp>`), `connect_pinned` (dials **only** a `&SafeIp`, so the compiler forbids a connect to an unvetted address), `perform_request` (content-length/chunked/close framing + gzip/deflate), and the redirect-following `fetch` that re-runs the SSRF gate on **every hop** — verified by loopback round-trips incl. that a loopback address is refused by default and reachable only through the explicit `allow_hosts` exemption. Plus **`htmlparse`** (stage 1 — core extraction): a dependency-free `html.parser` port pulling `<title>`, meta `description`, visible body text (script/style/noscript/template/svg/math excluded, nav/header/footer/aside/form boilerplate text dropped but its links kept), every `<a href>` in document order, `<link rel=canonical>`, `<base href>`, `meta robots`, and a stop-word language guess — cross-checked byte-identical to Python on realistic pages (the image/video verticals + JSON-LD/SPA recovery are stage 2). Plus the **crawl `frontier`** (a dependency-free port of the SQLite-backed queue): per-URL state (`queued`/`leased`/`done`/`error`/`skipped`), deterministic leasing (shallowest queued, ties by insertion order) with per-host politeness (`next_time`/`crawl_delay`/fetch counter), host budgets, the missing-host lease fallback, `reclaim` for resume, and a robots cache — the lease ordering + politeness + budgets cross-checked byte-identical to Python. Plus the **`index` store core** (a dependency-free port of the SQLite document store the crawler writes to): `content_hash` (SHA-256 exact-dup key over crawlcore), `upsert_document` (stable rowids + update-in-place), conditional-GET validators (`etag`/`last_modified`) + `touch_revalidated`, the recrawl due-list, the `(src → dst)` link graph with `recompute_incoming` counts, and index `stats` — `content_hash` + the store behaviour cross-checked byte-identical to Python (FTS5/BM25 search + verticals + PageRank are the ranking increment). And — tying it all together — the **resumable `crawler`**: lease → SSRF-checked fetch → htmlparse → index → capped frontier expansion, enforcing per-host + total + depth budgets, a content-type allowlist, URL canonicalization + frontier dedup, trap guards (segment-repeat / query-param / path-depth), `robots.txt` (fetched + cached per host, with `Crawl-delay`), per-host politeness, `rel=canonical` + `meta robots`, content-hash dedup, and conditional GET (304 → revalidate). Verified by **end-to-end loopback crawls** (a mock site drains, indexes the allowed pages, obeys a robots `Disallow`, records the link graph) plus that the crawl stays SSRF-gated (a loopback site is reachable only through the `allow_hosts` exemption). And now the query side — **`ranking`** (search): the query language (`parse_query`: quoted phrases, `+required`/`-excluded`/optional terms, and `site:`/`host:`/`lang:`/`filetype:`/`intitle:`/`before:`/`after:`/`date:`/`boost:`/`penalize:` operators reduced to structured filters, so no user text reaches a matcher as an operator), an *explicit* ranking (a field-weighted BM25 relevance base + link popularity + host authority + freshness + a proximity/phrase bonus + a content-quality anti-doorway term + per-query boost/penalize optics), cross-host near-duplicate collapse (SimHash), pagination + `fresh`/`only_files` verticals, and a query-biased **`make_snippet`** (HTML-escaped first, then `<mark>`-highlighted). The pure pieces — the parser, `freshness`/`proximity`/`content_quality`, and the snippet — are cross-checked **byte-identical** to Python; the BM25 base is a hand-rolled Okapi stand-in (behaviourally faithful — stdlib has no FTS5), unit-tested for ranking order. And — closing the loop — the **no-JS `serve`** layer: pure renderers + a pure `route()` over an `Arc<Mutex<Index>>` (search UI at `/` + `/search` with `?type=news`/`files` verticals, the `/api/search` JSON API the PHP bridge calls with `?limit`/`?page_size`/`?sort`, `/about` + `/stats`, `/opensearch.xml`, `/metrics`, `/healthz`, `/style.css`), with only the async accept loop behind `net` — every rendered field escaped and snippets XSS-safe (already-escaped, only `<mark>` survives); verified by route unit tests + a loopback HTTP round-trip of `/api/search`. And the **offline ranking signals** — `compute_pagerank` (PageRank-lite over the internal link graph → `doc.rank`) and `compute_host_authority` (cross-domain host-level PageRank, edge-weighted by distinct linking pages → `host_authority` + `doc.host_rank`, the authority term the ranker blends), driven by `finalize` (recompute-incoming → page-rank → host-authority) — the power iteration reproduced exactly, cross-checked **byte-identical** to Python. **79 tests**, zero third-party deps by default (net → tokio + getrandom only). The full crawl→index→search→**serve** pipeline runs end-to-end, now with the real authority signal. Next: htmlparse stage-2 verticals (needs a bounded JSON parser) + their result views + keep-alive `Fetcher` pool + federation + suggest/typeahead |
