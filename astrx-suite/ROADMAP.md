...
Six engines + one shared library, ported from the Python suite — **the migration
is complete and the Python is retired**: the tree contains no `.py` files at all
(the reference and the golden generators live in git history at the tag
`python-reference-final`; see `crates/GOLDENS.md`). Each engine is a crate that
