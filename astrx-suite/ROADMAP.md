...
async `socks5_connect`, loopback-tested) and the HTTP wire helpers have landed.
What remains: a hand-rolled gzip/deflate INFLATE (crawlcore, zero-dep) + the async
`perform_request` on top of it, then the fetcher — where taking an `&OnionHost`
makes the anti-leak invariant a compile guarantee — the resumable frontier, the
