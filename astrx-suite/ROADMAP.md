...
| **websearch**    | 🚧 kickoff | 9 | **SSRF gate as a type** — `SafeIp` wraps an `IpAddr` only after `ip_is_internal` clears it (loopback/private/link-local/reserved/multicast/unspecified, IPv4-mapped unwrapped, **fail-closed**), so the net-tier connect taking a `&SafeIp` makes an SSRF to an internal address a *compile* error; cross-checked byte-identical to Python over a 60+-case IPv4/IPv6 special-range corpus. Plus **`dedup`** (FNV-1a word-bigram-shingled 64-bit SimHash, reusing crawlcore's bit-math, cross-checked). Zero third-party deps by default | lift shared `urlparse` into crawlcore → `canonical`/`robots`; then the SSRF-checked HTTP client + resumable crawler + FTS index; BM25 + PageRank ranking; verticals; no-JS UI + JSON API; federation (sharding + scatter-gather) |
