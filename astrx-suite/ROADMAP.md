...
| **onioncrawler** | 🚧 in progress | 17 | **darknet host gate as a type** — `OnionHost`/`I2pHost`/`DarknetHost` construct-only-if-valid (`normalize_host`, v3/v2 onion, i2p b32/name, `is_darknet_host`) so a clearnet/localhost/IP leak is a *compile* error; in-text `.onion` discovery scanner (look-behind, 56/16, port clamp, path-stop set); stdlib language-guess — `onion` + `lang` cross-checked byte-identical to Python, zero third-party deps by default. **Next:** URL canonicalizer + abuse blocklist + entity index (pure), then the `net` tier: SOCKS5 fetcher + robots/sitemap + resumable frontier + Tor fetch-pool + no-JS search |
...
itself (`OnionHost`/`I2pHost`/`DarknetHost` + the `.onion` discovery scanner) and
the language-guess have landed, cross-checked byte-identical to Python; the URL
canonicalizer, abuse blocklist and the `net` tier (SOCKS5 fetcher, robots,
frontier) are next. Reuses crawlcore (robots/traps/dedup/scheduler) directly.
