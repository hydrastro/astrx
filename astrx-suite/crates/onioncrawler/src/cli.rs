...
use crate::abuse::{load_abuse_filter, AbuseFilter};
use crate::canonical::canonicalize;
use crate::crawler::{CrawlConfig, Crawler};
use crate::fetcher::Fetcher;
use crate::serve::{json_str, serve, RateLimits, SearchServer, ServeConfig};
use crate::store::{Caps, Enqueued, Reseed, Store};
use crate::submit::submit_many;
...
/// Run the `onioncrawler` command line and return the process exit code.
///
/// `args` is the argument list **without** `argv[0]`, so the caller decides what
/// the program was invoked as: `src/bin/onioncrawler.rs` passes
/// `std::env::args().skip(1)`, and the `astrx` multiplexer passes everything
/// after `astrx onioncrawler`. Both therefore see byte-identical parsing, help text
