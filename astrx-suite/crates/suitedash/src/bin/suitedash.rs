...
use suitedash::config::{apply_service_flags, load_config, Config, ConfigError};
use suitedash::metrics::summarize;
use suitedash::poller::poll_all;
use suitedash::render::render_status_json;
use suitedash::server::serve_config;
use suitedash::Monitor;
...
fn main() -> ExitCode {
    ...
    rt.block_on(run(config, args.check))
}

async fn run(config: Config, check: bool) -> ExitCode {
    if check {
