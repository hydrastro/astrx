...
use crate::search::{serve_search, SearchServer};
use crate::store::{Stats, Store, DEFAULT_SPAM_THRESHOLD};
use crate::tracker_http::serve_http_tracker;
use crate::tracker_udp::UdpTracker;
use crate::{default_bootstrap, Indexer, IndexerConfig};
...
struct SearchArgs {
    ...
    /// Log one access line per request. Off by default, so a server started the
    /// way it always was writes exactly what it always wrote.
    verbose: bool,
}
...
/// Run the `torrentds` command line and return the process exit code.
///
/// `args` is the argument list **without** `argv[0]`, so the caller decides what
/// the program was invoked as: `src/bin/torrentds.rs` passes
/// `std::env::args().skip(1)`, and the `astrx` multiplexer passes everything
/// after `astrx torrentds`. Both therefore see byte-identical parsing, help text
...
options:
  ...
  -v, --verbose           log one access line per request to stderr
  --log-format FMT        log as 'text' (default, human) or 'json'
                          (one object per line)
"
    )
}
...
mod tests {
    use super::*;
    use crate::bencode::{encode, Ben, Dict};
    use crate::infohash::sha1;
    use crate::metadata::parse_info;
