...
use crate::routing::{
    decode_nodes, decode_peers, distance, encode_endpoint, encode_nodes, random_node_id, InfoHash,
    Node, NodeId, RoutingTable, DEFAULT_K, ID_BYTES,
};
...
pub type InfohashSink = Arc<dyn Fn(&InfoHash, Option<SocketAddrV4>) + Send + Sync>;

/// The result of an outbound [`DhtNode::get_peers`].
#[derive(Debug, Clone, Default)]
pub struct GetPeersOutcome {
    /// Peers holding the infohash (the query's `values`).
    pub peers: Vec<SocketAddrV4>,
    /// Closer nodes to query next (the query's `nodes`).
    pub nodes: Vec<Node>,
    /// The announce `token`, if the peer returned one.
    pub token: Option<Vec<u8>>,
}

/// The result of an outbound [`DhtNode::sample_infohashes`] (BEP-51).
#[derive(Debug, Clone, Default)]
pub struct SampleOutcome {
    /// The sampled infohashes.
    pub samples: Vec<InfoHash>,
    /// Closer nodes to continue the walk.
    pub nodes: Vec<Node>,
    /// Total infohashes the peer holds (`num`), if reported.
    pub num: Option<i64>,
    /// Suggested re-sample interval in seconds, if reported.
...
impl RecentRing {
    fn remember(&mut self, ih: InfoHash) {
...
#[derive(Default)]
struct ServedPeers {
    order: VecDeque<InfoHash>, // infohash insertion order, for eviction
    map: HashMap<InfoHash, Vec<SocketAddrV4>>,
}
...
impl std::fmt::Debug for DhtConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DhtConfig")
            ...
            .field("on_infohash", &self.on_infohash.as_ref().map(|_| "<sink>"))
            .field("k", &self.k)
...
mod tests {
    ...
    async fn get_peers_then_announce_with_token() {
        ...
        let token = a.get_peers(&ih, b_addr).await.unwrap().token;
    ...
    async fn sample_infohashes_returns_recent() {
        ...
        let out = a.sample_infohashes(&[0u8; ID_BYTES], b_addr).await.unwrap();
        assert_eq!(out.interval, Some(SAMPLE_INTERVAL));
        assert_eq!(out.num, Some(2));
        assert!(out.samples.contains(&ih1));
        assert!(out.samples.contains(&ih2));
    }
    ...
    async fn iterative_lookup_finds_announced_peer() {
        ...
        let token = x.get_peers(&ih, c_addr).await.unwrap().token;
