...
/// A 160-bit node ID (the identity a DHT node advertises and routes by).
pub type NodeId = [u8; ID_BYTES];

/// A 20-byte BitTorrent infohash — a v1 SHA-1, or a truncated v2 SHA-256 on the
/// wire. Structurally identical to a [`NodeId`], but distinct in *intent*: an
/// infohash names a torrent, a node ID names a peer. Kept as an alias rather than
/// a newtype because both are raw wire bytes throughout the crate (a newtype would
/// add conversion noise to the byte-exact paths without preventing a real bug seen
/// today); the alias documents which 20 bytes a signature means.
pub type InfoHash = [u8; ID_BYTES];
...
/// A bounded set of nodes ordered least- to most-recently seen. The `k` bound is
/// enforced by [`KBucket::add`]; the fields are private so a caller cannot push
/// past it or reorder the recency invariant.
#[derive(Debug)]
pub struct KBucket {
    k: usize,
    nodes: Vec<Node>,
...
impl KBucket {
    ...
    /// The contacts, least- to most-recently seen (read-only).
    #[must_use]
...
impl RoutingTable {
    ...
    /// This table's own node ID (the root all distances are measured from).
    #[must_use]
