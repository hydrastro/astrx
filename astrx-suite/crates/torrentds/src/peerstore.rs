...
impl PeerStore {
    ...
    /// `(complete/seeders, incomplete/leechers, downloaded)`.
    pub fn counts(&mut self, infohash: &[u8; 20], now: u64) -> (u64, u64, u64) {
        self.reap(now);
        match self.swarms.get(infohash) {
            ...
            Some(sw) => {
                ...
                (complete, incomplete, sw.downloaded)
            }
        }
    }

    /// Alias for [`counts`](Self::counts) (the scrape triple).
    pub fn scrape(&mut self, infohash: &[u8; 20], now: u64) -> (u64, u64, u64) {
...
mod tests {
    ...
    fn peers_expire_after_ttl() {
        ...
        ps.announce(&IH, ep("1.2.3.4:1"), 0, Event::Started, 0);
        assert_eq!(ps.counts(&IH, 199), (1, 0, 0)); // still alive
        assert_eq!(ps.counts(&IH, 201), (0, 0, 0)); // reaped
    ...
    fn snapshot_restore_round_trip() {
        ...
        assert_eq!(restored.counts(&IH2, 1000), (1, 0, 1)); // downloaded preserved
