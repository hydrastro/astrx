...
///
/// Counts saturate to `i32::MAX` so a huge `downloaded` counter can't wrap to a
/// negative `i32` on the wire (Python raises `struct.error` and drops the reply; a
/// saturated max is the safer analogue). Taking [`ScrapeCounts`] rather than a bare
/// `(i32, i32, i32)` puts the BEP-15 field order under named control right here.
pub fn encode_scrape_response(txn: u32, entries: &[ScrapeCounts]) -> Vec<u8> {
    ...
    o.extend_from_slice(&txn.to_be_bytes());
    for c in entries {
        o.extend_from_slice(&clamp(c.complete).to_be_bytes());
        o.extend_from_slice(&clamp(c.downloaded).to_be_bytes());
        o.extend_from_slice(&clamp(c.incomplete).to_be_bytes());
    }
    o
}
...
impl std::fmt::Debug for UdpTracker {
    ...
    pub fn handle(&self, data: &[u8], src: SocketAddr, now: u64) -> Option<Vec<u8>> {
        ...
        match action {
            ...
            ACTION_ANNOUNCE => {
                ...
                let c = store.counts(&req.info_hash, now);
                ...
                Some(encode_announce_response(
                    txn,
                    interval,
                    c.incomplete as i32, // leechers
                    c.complete as i32,   // seeders
                    &peers,
                ))
            }
            ACTION_SCRAPE => {
                ...
                let mut store = self.store.lock().unwrap();
                // No positional reshuffle: `encode_scrape_response` reads the fields
                // by name and puts them in BEP-15 wire order (and saturates them).
                let entries: Vec<ScrapeCounts> =
                    hashes.iter().map(|h| store.counts(h, now)).collect();
                drop(store);
