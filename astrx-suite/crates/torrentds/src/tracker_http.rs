...
/// One scrape entry: `(infohash, (complete, downloaded, incomplete))`.
pub type ScrapeEntry = ([u8; 20], (i64, i64, i64));
...
pub fn scrape_response_bytes(entries: &[ScrapeEntry]) -> Vec<u8> {
    ...
    for (ih, (complete, downloaded, incomplete)) in entries {
        let mut e = Dict::new();
        e.insert(b"complete".to_vec(), Ben::Int(*complete));
        e.insert(b"downloaded".to_vec(), Ben::Int(*downloaded));
        e.insert(b"incomplete".to_vec(), Ben::Int(*incomplete));
...
fn handle_announce(params: &Query, src: SocketAddr, store: &Mutex<PeerStore>, now: u64) -> Vec<u8> {
    ...
    let (complete, incomplete, _dl) = st.counts(&info_hash, now);
...
fn handle_scrape(params: &Query, store: &Mutex<PeerStore>, now: u64) -> Vec<u8> {
    ...
    let mut st = store.lock().unwrap();
    let entries: Vec<([u8; 20], (i64, i64, i64))> = hashes
        .iter()
        .map(|h| {
            let (complete, incomplete, downloaded) = st.counts(h, now);
            (*h, (complete as i64, downloaded as i64, incomplete as i64))
        })
        .collect();
    drop(st);
