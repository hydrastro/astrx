...
/// A bencode dictionary: raw byte keys → values, kept sorted by `BTreeMap` so a
/// re-encode is always canonical. This is the inner type of [`Ben::Dict`], and the
/// shape KRPC `a`/`r` payloads and info-dicts are handled as throughout the crate.
