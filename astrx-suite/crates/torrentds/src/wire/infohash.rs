//! Byte-exact BitTorrent infohashes, and the hand-rolled hashes they use.
//!
//! SHA-1 (v1) and SHA-256 (BEP-52 v2) are kept in-crate so the crate has no
//! dependencies (std has no crypto). Both are verified byte-identical to Python's
//! `hashlib` against the FIPS/RFC vectors.
...
/// The BEP-52 v2 infohash: SHA-256 of the canonical bencoding of the info dict.
pub fn infohash_v2(info: &Ben) -> [u8; 32] {
    sha256(&encode(info))
}
...
/// Hand-rolled SHA-256 (FIPS 180-4). Verified byte-identical to Python's hashlib.
...
mod tests {
    ...
    fn sha256_fips_vectors() {
        ...
        // multi-block (two 64-byte blocks) — exercises the padding boundary.
        assert_eq!(
            hexs(&sha256(
