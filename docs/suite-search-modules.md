# AstrX suite integration — native CMS pages for astrx-suite

**Seven drop-in AstrX modules** that surface the [`astrx-suite`](../astrx-suite/)
engines as native, no-JS pages inside the CMS. Each is a thin, zero-dependency
PHP bridge to a standalone localhost Python service's JSON API — the search
bridges own **no database tables** (the crawl/index lives entirely in the
external engine); AstrX just makes a short loopback HTTP GET, sanitises every
field, and renders it through its own templates and escaping.
...
> **Where this lives.** This folder sits at the **AstrX repo root**, a sibling of
> `astrx-suite/` — not inside it — because every file below has a canonical home
> in the CMS tree (`src/AstrX/…`, `resources/…`, `src/setup/…`). Install is a
> one-step overlay (see [Files and where they go](#files-and-where-they-go-in-the-astrx-tree));
> the pages render inside the AstrX shell and inherit the site's exact theme.

## Separate pages by design (with an optional unified tab)

The bridges are intentionally **separate pages**, not one merged megasearch —
each with its own controller, template, config, lang domain, module manifest and
nav entry, so disabling or purging one never touches the others:

1. **Internal site search** — *existing, untouched.* Module `search`,
   `SiteSearchController` + `SiteSearchService`, slug `/search`. Full-text search
   over the CMS's own content (news, pages, comments, board posts) via the local
   `search_index` table. No network calls.
2. **Clear-web search** (`websearch`), **onion search** (`onionsearch`), and
   **torrent search** (`torrentsearch`) — each bridges to its own Python engine
   (the engine owns any Tor SOCKS hop; AstrX never speaks Tor).

If you *do* want one box over several engines, `fedsearch` provides exactly that
as an opt-in page with `?source=` tabs — without collapsing the standalone pages.
