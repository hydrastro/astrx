...
final class CommentController extends AbstractController
{
    ...
    private function renderComments(): void
    {
        ...
        $this->ctx->set('comments_order_asc',    !$descending, ContextScope::SHARED);
        $this->ctx->set('comments_order_desc',   $descending,  ContextScope::SHARED);
        $this->ctx->set('comments_indent_on',    $nested,      ContextScope::SHARED);
        $this->ctx->set('comments_indent_off',   !$nested,     ContextScope::SHARED);
        $this->ctx->set('comments_per_page_val', $csPerPage,   ContextScope::SHARED);

        // Pagination URLs generated by DefaultTemplateContext::finalise()
        $this->ctx->set('comments_any',   $comments !== [], ContextScope::SHARED);
        $this->ctx->set('comments',       $comments,        ContextScope::SHARED);
        $this->ctx->set('comments_count', $total,           ContextScope::SHARED);
