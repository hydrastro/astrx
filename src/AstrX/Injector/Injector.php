...
final class Injector
{
    ...
    /** @return Result<bool> */
    public function addHelper(object $helperInstance, string $helperMethod)
    : Result {
        ...
        try {
            ...
            if (!($p0 instanceof ReflectionNamedType) ||
                $p0->getName() !== 'object') {
                $d = Diagnostics::of(
                    new HelperInvalidSignatureDiagnostic(
                        self::ID_HELPER_INVALID_SIGNATURE,
            ...
            if (!($p1 instanceof ReflectionNamedType) ||
                $p1->getName() !== 'string') {
                $d = Diagnostics::of(
                    new HelperInvalidSignatureDiagnostic(
                        self::ID_HELPER_INVALID_SIGNATURE,
    ...
    public function setClassArgs(string $className, array $args)
    : void {
    ...
    public function getClass(string $className, bool $create = true)
    : Result {
        ...
        $d = Diagnostics::of(
            new ClassNotFoundDiagnostic(
                self::ID_CLASS_NOT_FOUND, self::LVL_CLASS_NOT_FOUND, $className,
            )
        );
    ...
    /** @return Result<object|null> */
    public function createClass(string $className, bool $share = true)
    : Result {
    ...
    public function getClassArg(string $className, string $argName)
    : mixed {
        $name = $this->getIndexName($className);

        if (!isset($this->classesArgs[$name]) ||
            !array_key_exists($argName, $this->classesArgs[$name])) {
            return null;
        }
